...
public:
    ...
    void addJobToPrinter(const Job& job); //REQUIRE (job.getJobNumber() >= 0, "Invalid job number"); ENSURE (printerjobs_.back().getJobNumber() == job.getJobNumber(), "Job number not added correctly");
    ...
    int getCO2Emissions(); //REQUIRE (CO2_emissions >= 0, "Invalid CO2 emissions value");
